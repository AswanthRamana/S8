...
def login():
    ...
    if request.method=='POST':
        ...
        cursor = mydb.cursor()
        cursor.execute('SELECT * FROM sc_company WHERE uname = %s AND pass = %s', (uname, pwd))
...
def reg_sup():
    ...
    if request.method=='POST':
        ...
        rd=str(rdate).split("-")
        rdd=rd[2]+"-"+rd[1]+"-"+rd[0]
        sql = "INSERT INTO sc_supplier(id,name,address,mobile,email,uname,pass,rdate) VALUES (%s, %s, %s, %s, %s, %s, %s, %s)"
        val = (id2,name,address,mobile,email,uname,pass1,rdd)
        cursor.execute(sql, val)
        mydb.commit()            
        print(cursor.rowcount, "Registered Success")
